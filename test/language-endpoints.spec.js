...
describe('Language Endpoints', function() {
   ...
   **/
	describe(`GET /api/language`, () => {
		const [ usersLanguage ] = testLanguages.filter((lang) => lang.user_id === testUser.id);
		const usersWords = testWords.filter((word) => word.language_id === usersLanguage.id);
   ...
   **/
	describe(`POST /api/language/guess`, () => {
		...
		context(`Given correct guess`, () => {
			const testLanguagesWords = testWords.filter((word) => word.language_id === testLanguage.id);
